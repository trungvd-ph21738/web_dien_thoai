...
<div>
    <div class="row">
        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <a href="/ram/view-add" class="btn btn-primary mb-3">Add Ram</a>
            <div class="card">
                <h5 class="card-header">Ram Table</h5>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered first">
                            ...
                            <tbody>
                            <c:forEach items="${ramList}" var="ram" varStatus="item">
                                <tr>
                                    <td>${item.index+1}</td>
                                    <td>${ram.ramCode}</td>
                                    <td>${ram.ramName}</td>
                                    ...
                                    <td>${ram.updatedAt}</td>
                                    <td>
                                        <c:if test="${ram.status == 1}">
                                            <span class="badge badge-success">Còn kinh doanh</span>
                                        </c:if>
                                        <c:if test="${ram.status == 0}">
                                            <span class="badge badge-danger">Dừng kinh doanh</span>
                                        </c:if>
                                    </td>
                                    <td>
                                        <a href="/ram/view-update?id=${ram.id}" class="btn btn-brand">Update</a>
                                        <a href="/ram/remove?id=${ram.id}" class="btn btn-danger">Delete</a>
                                    </td>
                                </tr>
                            </c:forEach>
                            </tbody>
