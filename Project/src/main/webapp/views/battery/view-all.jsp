...
<div>
    <div class="row">
        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <a href="/battery/view-add" class="btn btn-primary mb-3">Add Battery</a>
            <div class="card">
                <h5 class="card-header">Battery Table</h5>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered first">
                            ...
                            <tbody>
                            <c:forEach items="${batteryList}" var="battery" varStatus="item">
                                <tr>
                                    <td>${item.index+1}</td>
                                    <td>${battery.pinCode}</td>
                                    <td>${battery.pinName}</td>
                                    <td>${battery.createdAt}</td>
                                    <td>${battery.updatedAt}</td>
                                    <td>
                                        <c:if test="${battery.status == 1}">
                                            <span class="badge badge-success">Còn kinh doanh</span>
                                        </c:if>
                                        <c:if test="${battery.status == 0}">
                                            <span class="badge badge-danger">Dừng kinh doanh</span>
                                        </c:if>
                                    </td>
                                    <td>
                                        <a href="/battery/view-update?id=${battery.id}" class="btn btn-brand">Update</a>
                                        <a href="/battery/remove?id=${battery.id}" class="btn btn-danger">Delete</a>
                                    </td>
                                </tr>
                            </c:forEach>
                            </tbody>
