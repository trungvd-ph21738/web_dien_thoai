...
<div>
    <div class="row">
        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <a href="/chip/view-add" class="btn btn-primary mb-3">Add Chip</a>
            <div class="card">
                <h5 class="card-header">Chip Table</h5>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered first">
                            ...
                            <tbody>
                            <c:forEach items="${chipList}" var="chip" varStatus="item">
                                <tr>
                                    <td>${item.index+1}</td>
                                    <td>${chip.chipCode}</td>
                                    <td>${chip.chipName}</td>
                                    <td>${chip.createdAt}</td>
                                    <td>${chip.updatedAt}</td>
                                    <td>
                                        <c:if test="${chip.status == 1}">
                                            <span class="badge badge-success">Còn kinh doanh</span>
                                        </c:if>
                                        <c:if test="${chip.status == 0}">
                                            <span class="badge badge-danger">Dừng kinh doanh</span>
                                        </c:if>
                                    </td>
                                    <td>
                                        <a href="/chip/view-update?id=${chip.id}" class="btn btn-brand">Update</a>
                                        <a href="/chip/remove?id=${chip.id}" class="btn btn-danger">Delete</a>
                                    </td>
                                </tr>
                            </c:forEach>
                            </tbody>
