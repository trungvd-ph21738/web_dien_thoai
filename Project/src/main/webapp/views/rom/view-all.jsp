...
<div>
    <div class="row">
        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <a href="/rom/view-add" class="btn btn-primary mb-3">Add Rom</a>
            <div class="card">
                <h5 class="card-header">Rom Table</h5>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered first">
                            ...
                            <tbody>
                            <c:forEach items="${romList}" var="rom" varStatus="item">
                                <tr>
                                    <td>${item.index+1}</td>
                                    <td>${rom.romCode}</td>
                                    <td>${rom.romName}</td>
                                    ...
                                    <td>${rom.updatedAt}</td>
                                    <td>
                                        <c:if test="${rom.status == 1}">
                                            <span class="badge badge-success">Còn kinh doanh</span>
                                        </c:if>
                                        <c:if test="${rom.status == 0}">
                                            <span class="badge badge-danger">Dừng kinh doanh</span>
                                        </c:if>
                                    </td>
                                    <td>
                                        <a href="/rom/view-update?id=${rom.id}" class="btn btn-brand">Update</a>
                                        <a href="/rom/remove?id=${rom.id}" class="btn btn-danger">Delete</a>
                                    </td>
                                </tr>
                            </c:forEach>
                            </tbody>
